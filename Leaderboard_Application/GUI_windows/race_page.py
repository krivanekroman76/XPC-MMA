...
from core import ConfigManager, tr
...
class RacePage(QWidget):
    ...
    def _setup_queue_panel(self, parent_layout):
        ...
        self.btn_complete_race = QPushButton()
        self.btn_complete_race.setObjectName("successButton")
        self.btn_complete_race.setMinimumHeight(40)
        self.btn_complete_race.clicked.connect(self.complete_race)
        queue_layout.addWidget(self.btn_complete_race)
    ...
    def complete_race(self):
        """Marks the race as completed and sends FCM notifications to subscribers."""
        ...
        reply = QMessageBox.question(self, tr.t("btn_confirm"), tr.t("race_confirm_complete"))
        if reply != QMessageBox.Yes:
            return
        
        # Call the firebase method
        success, message = self.db.complete_race(league_id, race_id, race_name)
        if success:
            QMessageBox.information(self, tr.t("msg_success"), message)
        else:
    ...
    def retranslate_ui(self):
        ...
        self.btn_complete_race.setText(tr.t("race_btn_complete"))
    ...
    def load_recovery(self):
        ...
        try:
            recovery_data = ConfigManager.load_json(RECOVERY_FILE)
            if not recovery_data:
                return  # <--- WE ADDED THIS! Now Python knows to exit if there's no data.
